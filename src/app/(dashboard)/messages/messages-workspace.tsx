...
type Inbox = { role: "institute-admin" | "institute-staff" | "student"; selectedId: string | null; conversations: { id: string; peer: { id: string; name: string; email: string }; latestMessageAt: Date | null; latestMessagePreview: string; unread: boolean }[]; messages: { id: string; senderId: string; senderName: string; body: string; deleted: boolean; createdAt: Date }[]; peers: { id: string; name: string; email: string }[] };

const TITLE_BY_ROLE: Record<Inbox["role"], string> = {
  "institute-admin": "Staff messages",
  "institute-staff": "Messages",
  student: "Teacher messages",
};

const PEER_LABEL_BY_ROLE: Record<Inbox["role"], string> = {
  "institute-admin": "Select active staff",
  "institute-staff": "Select a person",
  student: "Select active teacher",
};

export function MessagesWorkspace({ role, selectedId, conversations, messages, peers }: Inbox) {
  const [body, setBody] = useState(""); const [peerId, setPeerId] = useState("");
  ...
  const create = () => { const data = new FormData(); data.set("peerId", peerId); startTransition(() => startAction(data)); };
  return <div className="flex flex-col gap-6"><div><h1 className="text-2xl font-semibold">{TITLE_BY_ROLE[role]}</h1><p className="text-sm text-muted-foreground">Private conversations within your institute.</p></div>
    <Card><CardHeader><CardTitle>Start a conversation</CardTitle></CardHeader><CardContent className="flex flex-col gap-2 sm:flex-row"><select value={peerId} onChange={(event) => setPeerId(event.target.value)} className="h-9 flex-1 rounded-md border bg-background px-2 text-sm"><option value="">{PEER_LABEL_BY_ROLE[role]}</option>{peers.map((member) => <option key={member.id} value={member.id}>{member.name} ({member.email})</option>)}</select><Button type="button" disabled={!peerId || starting} onClick={create}>{starting ? "Opening..." : "Open conversation"}</Button></CardContent></Card>
