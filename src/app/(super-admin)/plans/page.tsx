...
export default async function PlansPage() {
  const [plans, distribution] = await Promise.all([
    listPlans(),
    getPlanDistribution(),
  ]);
  const subscribersByPlan = new Map(
    distribution.map((item) => [String(item.planId), item.count]),
  );
  const totalSubscribers = distribution.reduce(
  ...
  const rows: DataTableRow[] = plans.map((plan) => ({
    ...
    sortValues: [
      plan.name,
      plan.price,
      subscribersByPlan.get(String(plan._id)) ?? 0,
      null,
    ...
    cells: [
        ...
        trigger={
          <span className="block min-w-44">
            ...
            <span className="mt-0.5 block text-xs text-muted-foreground">
              /{plan.slug} · {plan.description || "No description"}
            </span>
          </span>
        }
      />,
      <div key="price">
        <p className="font-medium">{formatLkr(plan.price)}</p>
      ...
      <div key="adoption">
        <p className="font-medium tabular-nums">
          {subscribersByPlan.get(String(plan._id)) ?? 0} institutes
        </p>
        <p className="text-xs text-muted-foreground">Currently subscribed</p>
      </div>,
      <div key="entitlements" className="max-w-56">
        <p className="text-sm">
          {plan.features?.slice(0, 2).join(" · ") || "Core learning tools"}
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          {plan.limits?.maxStudents
            ? `Up to ${plan.limits.maxStudents} students`
            : "Unlimited students"}
        </p>
      </div>,
    ...
    <div className="flex flex-col gap-6">
      <div className="relative overflow-hidden rounded-[28px] border border-primary/15 bg-[linear-gradient(120deg,color-mix(in_oklch,var(--primary),transparent_91%),transparent_55%),var(--card)] px-5 py-6 sm:px-7 sm:py-8">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-eyebrow text-primary">Commercial catalog</p>
            <h1 className="text-heading mt-2 text-3xl">
              Plans built for growth.
            </h1>
            <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
              Shape the packages institutes see, then monitor adoption and
              entitlement clarity from one place.
            </p>
          </div>
          <PlanFormDialog />
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          icon={BadgeCheck}
          value={activePlans}
          sub={`${publicPlans} visible publicly`}
          tone="success"
        />
        <StatCard
        ...
        />
        <StatCard
          label="Unpublished plans"
          icon={Plus}
          value={plans.length - publicPlans}
          sub="Internal or private offers"
          tone="warning"
        />
      </div>
      <DataTableCard
        title="Plan catalog"
        sub="Edit a plan to refine its pricing, limits, or included features."
        columns={COLUMNS}
        rows={rows}
        searchPlaceholder="Search plans or slugs..."
        emptyTitle="No plans yet."
        emptyDescription="Create a plan to start packaging the platform."
        pageSize={12}
        filters={[
          {
            key: "status",
