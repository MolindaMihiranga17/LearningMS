...
export default async function InvoicesPage() {
  const [invoices, institutesList] = await Promise.all([
    listInvoices(),
    listInstitutes(),
  ]);
  const institutes = institutesList.map((institute) => ({
  ...
  const rows: DataTableRow[] = invoices.map((invoice) => {
    ...
    return {
      ...
      searchValue: `${invoice.invoiceNumber} ${institute?.name ?? ""} ${status}`,
      sortValues: [
        null,
        institute?.name ?? "",
        invoice.amount,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manual subscription billing records.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <a
            href="/api/platform-reports/invoices?format=csv"
            target="_blank"
            rel="noreferrer"
          ...
          <a
            href="/api/platform-reports/invoices?format=pdf"
            target="_blank"
            rel="noreferrer"
            className={cn(buttonVariants({ variant: "secondary", size: "sm" }))}
          >
          ...
          <InvoiceFormDialog institutes={institutes} />
        </div>
      </div>
      <div className="mt-6">
        <DataTableCard
