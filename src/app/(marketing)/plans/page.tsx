      ...
      <section className="mx-auto max-w-7xl px-5 pb-18 sm:px-7 sm:pb-24">
        {plans.length === 0 ? (
          ...
          <div className={`grid gap-5 ${plans.length === 1 ? "mx-auto max-w-md" : plans.length === 2 ? "mx-auto max-w-4xl md:grid-cols-2" : "lg:grid-cols-3"}`}>
            {plans.map((plan) => (
              <article key={plan.id} className={`relative flex flex-col rounded-[1.7rem] border bg-white p-6 shadow-xl shadow-slate-950/5 sm:p-7 ${plan.isRecommended ? "border-blue-400 ring-4 ring-blue-100/80" : "border-slate-200"}`}>
                {plan.isRecommended ? <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-blue-600 px-3 py-1 text-[11px] font-bold tracking-wide text-white uppercase">Recommended</span> : null}
                ...
                <Link href={session?.role === "institute-admin" ? `/plans/choose/${plan.slug}` : session ? "/dashboard" : "/login"} className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-700">{session?.role === "institute-admin" ? `Choose ${plan.name}` : session ? "Open workspace" : "Sign in to choose"} <ArrowRight className="size-4" /></Link>
              </article>
            ))}
          </div>
        )}
        <p className="mt-10 flex items-center justify-center gap-2 text-center text-sm text-slate-600"><CheckCircle2 className="size-4 shrink-0 text-emerald-600" />Eligible institute administrators can continue to secure PayHere checkout.</p>
      </section>
    </main>
  );
}
